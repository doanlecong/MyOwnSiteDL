    ...
    <table class="table">
        ...
        @foreach($myblogs as $blog)
            <tr>
                ...
                <td title="{{ $blog->title }}">{{ mb_substr($blog->title,0,50) }}</td>
                <td title="{{ $blog->slug }}">{{ mb_substr($blog->slug,0, 30) }}</td>
                <td title="{{strip_tags($blog->description)}}">{{ mb_substr(strip_tags($blog->description),0, 50,'UTF-8') }}</td>
                <td>
                    @if($blog->countView)
                ...
                <td>
                    <span class="disqus-comment-count" data-disqus-identifier="chuyende_{{$blog->slug.".html"}}">First article</span>
                </td>
                <td>
